...
class SaveLogosJob extends Job implements SelfHandling
{
    use DispatchesJobs;
    private $slide_id;
    private $logos;
     ...
     * @return mixed
     */
    public function handle(Image $image, Slide $slides)
    {
        ...
        foreach ($this->logos as $logo) {
            ...
            if ( !$logo['attachedLogoId'] && !$logo['detachedLogoId']) {
                if ( !array_key_exists('id', $logo)) {
                    ...
                    $this->dispatchFromArray(UpdateImageJob::class, $logo);
                }
            } else {
                ...
                if ($logo['detachedLogoId']) {
                    ...
                    $image = Image::find($logo['detachedLogoId']);
    ...
    {
        $newLogo = $this->dispatchFromArray(CreateImageJob::class, $logo);
